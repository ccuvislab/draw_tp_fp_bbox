...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        save_dir = "/home/u1755025/FedMPEN_mycode/output/class_predict/"
        ...
        if self._config_file.MODEL.SAVE_TP_FP_FN == True:
            ...
            if not os.path.exists(save_bb):
                os.makedirs(save_bb)
            store_path_name = self._config_file.MODEL.STORE_TP_FP_FN_FILE_NAME  
            output_file = os.path.join(save_bb, f"{store_path_name}_bb.txt")
            with open(output_file, "w") as file:
                for key, value in image_tp_fp_fn_bb.items():
                    tp_str = ', '.join(map(str, value['tp']))  
                    fp_str = ', '.join(map(str, value['fp']))
                    fn_str = ', '.join(map(str, value['fn']))
    ...
    for d in range(nd):
        ...
        if ovmax > ovthresh:
            if not R["difficult"][jmax]:
                if not R["det"][jmax]:
                    #這邊AP只記錄一個ovthresh，因為bbox一樣只記錄一個ovthresh
                    # if ovthresh == store_ovthresh:
                    tp[d] = 1.0
                    R["det"][jmax] = 1

                    #檢查該bb是否重複檢查以及只記錄一個ovthresh，若重複則不累加tp
                    if ovthresh == store_ovthresh and configfile.MODEL.SAVE_TP_FP_FN == True:
                        if bb_list not in store_tp_repeat:
                            store_tp_repeat.append(bb_list)
                            image_tp_fp_fn_bb[image_ids[d]]["tp"].append(bb_list)
                    ...
                    #檢查該bb是否重複檢查以及只記錄一個ovthresh，若重複則不累加fp
                    if ovthresh == store_ovthresh and configfile.MODEL.SAVE_TP_FP_FN == True:
                        if bb_list not in store_fp_repeat:
                            store_fp_repeat.append(bb_list)
                            image_tp_fp_fn_bb[image_ids[d]]["fp"].append(bb_list)
        ...
        #檢查該bb是否重複檢查以及只記錄一個ovthresh，若重複則不累加fn
        if ovthresh == store_ovthresh and configfile.MODEL.SAVE_TP_FP_FN == True:
            ...
            for i in range(len(R["det"])):
                ...
                #這邊發現即使只想儲存R["det"][i]=false的bbox，也還是有參雜R["det"][i]=1的bbox(tp bbox)，不知道為甚麼?
                if not R["det"][i] and not R["difficult"][i]:
                    if BBGT_list[i] not in store_fn_repeat:
                        store_fn_repeat.append(BBGT_list[i])
                        image_tp_fp_fn_bb[image_ids[d]]["fn"].append(BBGT_list[i])
                        image_tp_fp_fn_count[image_ids[d]]["fn"] += 1
                #刪除多出來的tp bbox(tp + fn = GT)
                for TP in tp_bbox:
                    if TP in image_tp_fp_fn_bb[image_ids[d]]["fn"]:
                        image_tp_fp_fn_bb[image_ids[d]]["fn"].remove(TP)
                        image_tp_fp_fn_count[image_ids[d]]["fn"] -= 1
